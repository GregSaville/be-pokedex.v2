...
class WelcomeController {
    ...
    val welcomeHTML = "<h1>Welcome to the Pokedex API</h1>" +
            " <h2>---Public Endpoints---</h2> <h4>/welcome</h4> <h4>/api/pokemon</h4>   <h4>/api/pokemon/{pokemon id}</h4> <h4>/api/pokemon/?name={pokemon's name}</h4> <h4>/api/pokemon/?type={type1,type2}  || ?type={type1 type2}</h4> <h4> for all with split typing --> /api/pokemon/?type={type}</h4> <h4>for all with only single typing --> /api/pokemon/?type={type }</h4>" +
